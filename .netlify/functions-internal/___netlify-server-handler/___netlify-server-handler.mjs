...
} from '/var/task/job-engine/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '/var/task/job-engine/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('/var/task/job-engine')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '/var/task/job-engine',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('/var/task/job-engine/.netlify/dist/run/handlers/server.js')
